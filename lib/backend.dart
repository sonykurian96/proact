...
SpecificQuestions specificQuestionsList = Get.find();
...
List<Node> commonQuestions = [
  ...
  Node("1", "Do you face any of the following problems during your periods?", [
...
//     PCOS
  [
    ...
    Node("1", " Are you noticing dark patches of skin in following regions:", [
      "Behind the neck",
      "Under armpits",
      "Under breasts",
      "None of the above"
...
//   Chlaymydia
  [
    ...
    Node(
        "1", " Have you noticed the following symptoms in the Rectum region?", [
      "Bleeding",
      "Rectal pain",
      "Rectal discharge",
      "None"
...
List<int> sample_list = [1, 2, 1, 2, 1, 2, 1, 2, 1];

void calculate_score_of_commonQuestions(List<int> a_list) {
  int index = 0;

  for (var ques in commonQuestions) {
    int selectedOption = a_list[index];
...
void displaySpecificQuestions() {
  ...
  for (int x = 0; x < 2; x++) {
    while (index[scores[x].diseasesID][0] < index[scores[x].diseasesID][1]) {
      ...
      specificQuestionsList.putspecificQuestions(ques.question);
      specificQuestionsList.putspecificResponses("$qNo");
      specificQuestionsList.questionNo.value = qNo;

      for (var opt in ques.responses) {
        specificQuestionsList.putspecificResponses(opt);
